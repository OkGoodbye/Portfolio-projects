...
class Player:
    ...
    def draw(self,screen):
        ...
        for i in range(self.resolution):
            ...
            if collides!=float("inf"):
                ...
                draw_image = pygame.Surface((line_width,collision_check[1].texture.get_height()))
                
                draw_image.blit(collision_check[1].texture,(-texture_slice,0))
                draw_image = pygame.transform.scale(draw_image,(line_width,h))
                screen.blit(draw_image,draw_image.get_rect(center = (draw_point,250)))
    ...
    def move(self,keys,rotate):
        ...
        normal_trial = (self.pos[0]-math.cos(self.angle)*fb/30,self.pos[1]-math.sin(self.angle)*fb/30)
        
        if not self.collide_all(normal_trial):
           
            self.pos = normal_trial
        tangent_trial = (self.pos[0]+math.cos(self.angle+math.pi/2)*lr/60,self.pos[1]+math.sin(self.angle+math.pi/2)*lr/60)
...
player = Player((7,3),math.pi/2,b_list)
